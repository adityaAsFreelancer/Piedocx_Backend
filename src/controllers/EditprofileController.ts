  ...
  try {
    ...
    if (!name || !email || !phone) {
      return res.status(400).json(
        createResponse(res, 400, 'All fields are required', [], false, true)
      );
    }
    ...
    if (!student) {
      return res.status(404).json(
        createResponse(res, 404, 'Student not found', [], false, true)
      );
    }
    ...
    if (req.files?.photo) {
      ...
      if (!allowedTypes.includes(photo.mimetype)) {
        return res.status(400).json(
          createResponse(res, 400, 'Only JPG, JPEG, PNG formats are allowed', [], false, true)
        );
      }
    ...
    return res.status(200).json(
      createResponse(res, 200, 'Profile updated successfully', student, true, false)
    );
  } catch (err) {
    console.error('Edit Profile Error:', err);
    return res.status(500).json(
      createResponse(res, 500, 'Something went wrong', [], false, true)
    );
  }
};
